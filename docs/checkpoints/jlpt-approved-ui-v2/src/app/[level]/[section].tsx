...
import N1ExamPicker from '@/components/jlpt/N1ExamPicker';
import N1OfficialTrial from '@/components/jlpt/N1OfficialTrial';
import { JlptExamHeader } from '@/components/jlpt/ui/JlptExamUI';
import {
...
export default function JlptSectionScreen() {
    ...
    const [selectedN1ExamId, setSelectedN1ExamId] = useState<string | null>(null);
    ...
    if (section === 'test') {
        if (level === 'N1') {
            const selectedExam = APPROVED_N1_EXAMS.find((exam) => exam.id === selectedN1ExamId);
            return <SafeAreaView style={styles.testContainer}>{selectedExam
                ? <N1OfficialTrial exam={selectedExam} onExit={()=>setSelectedN1ExamId(null)} />
                : <N1ExamPicker exams={APPROVED_N1_EXAMS} onChoose={(exam)=>setSelectedN1ExamId(exam.id)} onBack={()=>router.back()} />}
            </SafeAreaView>;
        }
        return <SafeAreaView style={styles.testContainer}><JlptExamHeader title={`${level} · JLPT模擬試験`} subtitle="日本語能力試験" onBack={()=>router.back()} /><JlptMockTest level={level}/></SafeAreaView>;
    }
