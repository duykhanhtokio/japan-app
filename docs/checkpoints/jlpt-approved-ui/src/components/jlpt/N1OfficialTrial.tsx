...
import { N1_2012_07_TRIAL, type TrialQuestion } from '@/data/jlpt-official/n1-2012-07-trial';
import { JLPT_EXAM, type JlptFontScale } from '@/theme/jlpt-exam-design-system';
import { clearN1TrialSession, loadN1TrialSession, saveN1TrialSession, type N1TrialSession } from '@/services/jlpt-trial-session-storage';

type Mode = 'exam' | 'practice';
const FONT_SCALES: readonly JlptFontScale[] = [0.9, 1, 1.1, 1.2, 1.3, 1.4];
const LISTENING = N1_2012_07_TRIAL.filter((question) => question.family === 'listening');
const VISUAL_OPTIONS: Readonly<Record<12 | 13, ImageSourcePropType>> = {
...
export default function N1OfficialTrial({ onExit }: { onExit: () => void }) {
  ...
  const [currentQuestion, setCurrentQuestion] = useState(N1_2012_07_TRIAL[0].id);
  ...
  const player = useAudioPlayer(N1_2012_07_AUDIO, { updateInterval: 100 });
  ...
  useEffect(() => {
    let active = true;
    void loadN1TrialSession().then((session) => {
  ...
  function resetLocalState() {
    ...
    setReviewing(false);
    setSubmittedAt(null);
    setCurrentQuestion(N1_2012_07_TRIAL[0].id);
    setInitialScrollY(0);
    scrollRef.current?.scrollTo({ y: 0, animated: false });
  }

  function persist(overrides: Partial<Parameters<typeof saveN1TrialSession>[0]> = {}) {
    return saveN1TrialSession({ answers, mode, status: submitted ? reviewing ? 'reviewing' : 'submitted' : started ? 'in_progress' : 'not_started', started, submitted, currentQuestion, scrollY: initialScrollY, playedAudioSegments, submittedAt, result: submitted ? calculateResult(answers) : null, ...overrides });
  }
  ...
  function begin() {
    ...
    void clearN1TrialSession().then(() => saveN1TrialSession({ answers: {}, mode, status: 'in_progress', started: true, submitted: false, currentQuestion: N1_2012_07_TRIAL[0].id, scrollY: 0, playedAudioSegments: [], submittedAt: null, result: null }));
  }
  ...
  function continueSession() {
    ...
    setSubmitted(false);
    setCurrentQuestion(pendingSession.currentQuestion || N1_2012_07_TRIAL[0].id);
  ...
  function submit() {
    ...
    setSubmittedAt(now);
    void persist({ status: 'submitted', submitted: true, submittedAt: now, result: calculateResult(answers) });
  }
  ...
  function requestSubmit() {
    if (answeredIds.size < N1_2012_07_TRIAL.length) setSubmitConfirmationVisible(true);
    else submit();
  }
  ...
  if (!started) return <View style={styles.startScreen}><JlptExamHeader title="N1 · JLPT模擬試験" subtitle="日本語能力試験" onBack={onExit} /><ScrollView contentContainerStyle={styles.startPage}>
    <JlptPaper>
      <Text style={styles.examName}>日本語能力試験 N1</Text>
      <Text style={styles.trialName}>2012年7月・第1回</Text>
      <View style={styles.rule} />
      <Text style={styles.startCopy}>原本と照合済みの全106問を収録しています。解答と聴解スクリプトは提出後の詳しい解説で確認できます。</Text>
      ...
      <JlptActionButton label="第1回を始める" onPress={begin} style={styles.startAction} />
    ...
    {pendingSession ? <JlptResumePrompt
      ...
      total={N1_2012_07_TRIAL.length}
      currentLabel={questionPositionLabel(pendingSession.currentQuestion)}
  ...
  const written = N1_2012_07_TRIAL.filter((question) => question.family !== 'listening');

  const correctCount = N1_2012_07_TRIAL.filter((question) => answers[question.id] === question.correctOptionId).length;
  const unansweredCount = N1_2012_07_TRIAL.filter((question) => !answers[question.id]).length;
  const wrongCount = N1_2012_07_TRIAL.length - correctCount - unansweredCount;

  if (submitted && !reviewing) return <View style={styles.screen}>
    <JlptExamHeader title="N1 · 試験結果" subtitle="2012年7月・第1回" onBack={onExit} />
    <ScrollView contentContainerStyle={styles.resultPage}><JlptPaper>
      ...
      <View style={styles.scoreGrid}>
        ...
        <ResultMetric label="正答率" value={`${Math.round((correctCount / N1_2012_07_TRIAL.length) * 100)}%`} />
      </View>
      <Text style={styles.rawScore}>正答数：{correctCount}/{N1_2012_07_TRIAL.length}</Text>
  ...
  if (submitted && reviewing) {
    const filtered = N1_2012_07_TRIAL.filter((question) => reviewFilter === 'all' || reviewFilter === 'wrong' && !!answers[question.id] && answers[question.id] !== question.correctOptionId || reviewFilter === 'unanswered' && !answers[question.id]);
      ...
      <ScrollView contentContainerStyle={styles.content}><JlptPaper>
        {filtered.length ? filtered.map((question, index) => {
          ...
          return <View key={question.id}>
            ...
            <QuestionBlock question={question} scale={fontScale} selected={answers[question.id]} submitted showPassage={firstForPassage} showTranscript={!question.audio || firstForSegment} onChoose={choose} onLayout={() => undefined} onFocus={() => undefined} />
          </View>;
  ...
  return <View style={styles.screen}>
    ...
    <View style={styles.toolRow}>
      <View><Text style={styles.progress}>{answeredIds.size}/{N1_2012_07_TRIAL.length} 回答済み</Text><Text style={styles.modeIndicator}>{mode === 'exam' ? '試験モード' : '練習モード'}</Text></View>
    ...
    <JlptProgressBar answered={answeredIds.size} total={N1_2012_07_TRIAL.length} />
    <ScrollView ref={scrollRef} contentOffset={{ x: 0, y: initialScrollY }} onMomentumScrollEnd={saveScroll} contentContainerStyle={styles.content}>
      <JlptPaper>
        ...
        {written.map((question, index) => {
          ...
          const firstForPassage = !question.passageId || previous?.passageId !== question.passageId;
          return <QuestionBlock key={question.id} question={question} scale={fontScale} selected={answers[question.id]} submitted={false} showProblemHeading={firstInProblem} showInstruction={firstInProblem} showPassage={firstForPassage} onChoose={choose} onLayout={(event) => registerPosition(question.id, event)} onFocus={() => setCurrentQuestion(question.id)} />;
        })}
        ...
        {LISTENING.map((question, index) => {
          ...
          return <View key={question.id}>
            ...
            <QuestionBlock question={question} scale={fontScale} selected={answers[question.id]} submitted={false} showProblemHeading={false} showInstruction={false} showPassage={false} onChoose={choose} onLayout={(event) => registerPosition(question.id, event)} onFocus={() => setCurrentQuestion(question.id)} />
          </View>;
        })}
        <JlptActionButton label="答案を提出する" onPress={requestSubmit} style={styles.submit} />
      </JlptPaper>
    </ScrollView>
    <JlptQuestionNavigator visible={navigatorVisible} labels={N1_2012_07_TRIAL.map((question) => question.id)} answered={answeredIds} current={currentQuestion} onChoose={goToQuestion} onClose={() => setNavigatorVisible(false)} />
    <JlptSubmitConfirmation visible={submitConfirmationVisible} total={N1_2012_07_TRIAL.length} answered={answeredIds.size} onCancel={() => setSubmitConfirmationVisible(false)} onSubmit={submit} />
  </View>;
}

function QuestionBlock({ question, scale, selected, submitted, showProblemHeading = true, showInstruction = true, showPassage = true, showTranscript = true, onChoose, onLayout, onFocus }: { question: TrialQuestion; scale: number; selected?: string; submitted: boolean; showProblemHeading?: boolean; showInstruction?: boolean; showPassage?: boolean; showTranscript?: boolean; onChoose: (questionId: string, optionId: string) => void; onLayout: (event: LayoutChangeEvent) => void; onFocus: () => void }) {
  return <View onLayout={onLayout} style={styles.questionBlock}>
    ...
    {question.visualOptionPage ? <Image source={VISUAL_OPTIONS[question.visualOptionPage]} resizeMode="contain" style={[styles.visualOptions, question.visualOptionPage === 12 ? styles.visualOptionsPage12 : styles.visualOptionsPage13]} accessibilityLabel={`${question.label}の選択肢図`} /> : null}
    ...
    {submitted ? <><JlptReviewFeedback correct={selected === question.correctOptionId} answer={question.correctOptionId} sourcePage={question.sourcePage} /><Text style={styles.sourceReference}>問題PDF：{question.sourcePage}ページ　解答PDF：{question.answerSourcePage}ページ</Text><Text style={styles.explanationPending}>この問題の詳しい解説は現在準備中です。{`\n`}Phần giải thích chi tiết của câu này đang được chuẩn bị.</Text>{showTranscript && question.audio?.transcriptJa ? <View style={styles.transcriptBlock}><Text style={styles.transcriptTitle}>聴解スクリプト（解答PDF {question.audio.transcriptSourcePage}ページ）</Text><Text style={styles.transcript}>{question.audio.transcriptJa}</Text></View> : null}</> : null}
  </View>;
}
...
function calculateResult(answers: Record<string, string>) {
  const correct = N1_2012_07_TRIAL.filter((question) => answers[question.id] === question.correctOptionId).length;
  const unanswered = N1_2012_07_TRIAL.filter((question) => !answers[question.id]).length;
  return { correct, wrong: N1_2012_07_TRIAL.length - correct - unanswered, unanswered, total: N1_2012_07_TRIAL.length };
}
...
function questionPositionLabel(questionId: string) {
  const index = N1_2012_07_TRIAL.findIndex((question) => question.id === questionId);
  const question = N1_2012_07_TRIAL[index];
  return question ? `${question.label}（${index + 1}/${N1_2012_07_TRIAL.length}）` : '先頭';
}
