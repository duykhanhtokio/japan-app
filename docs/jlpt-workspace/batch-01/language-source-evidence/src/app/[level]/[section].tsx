...
export default function JlptSectionScreen() {
    ...
    if (section === 'test') return <SafeAreaView style={styles.testContainer}>{level === 'N1' ? <N1OfficialTrial onExit={()=>router.back()} /> : <><JlptExamHeader title={`${level} · JLPT模擬試験`} subtitle="日本語能力試験" onBack={()=>router.back()} /><JlptMockTest level={level}/></>}</SafeAreaView>;
