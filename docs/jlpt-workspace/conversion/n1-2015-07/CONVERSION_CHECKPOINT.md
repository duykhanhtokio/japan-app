...
- Status: `LISTENING_CANDIDATE_IN_PROGRESS` (written source available 69/70 with question 29 blocked; listening candidate 13/37). This is not full-exam `structured_ready`.
...
Next: keep question 29 `BLOCKED_SOURCE_UNREADABLE`; do not infer it from the answer key. Continue with listening problem 3 using the repository audio and answer-script sources. Preserve candidate/unverified provenance and validate all seconds-to-milliseconds conversions, range bounds, ordering, overlap, response mapping, counts, and closing-announcement exclusion.
