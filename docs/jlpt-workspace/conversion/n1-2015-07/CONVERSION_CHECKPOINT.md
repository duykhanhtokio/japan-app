...
- Status: `LISTENING_CANDIDATE_COMPLETE` (written source available 69/70 with question 29 blocked; listening candidate 37/37 across 36 unique audio segments). This is not full-exam `structured_ready`.
...
Next: keep question 29 `BLOCKED_SOURCE_UNREADABLE`; do not infer it from the answer key. Assemble the available 69 written questions and 37 listening responses into the candidate structured dataset and adapter without changing the locked UI. Preserve all listening timing as candidate/unverified and retain the local written blocker in the checkpoint.
