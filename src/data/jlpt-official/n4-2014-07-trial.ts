...
const DATASET = require('./n4-2014-07/exam.candidate.json') as Dataset;
...
export const N4_2014_07_SESSION_KEY = 'jlpt:n4:2014-07:exam-05:session:v1';
export const N4_2014_07_TRIAL: readonly TrialQuestion[] = DATASET.questions.map((question) => ({
...
if (N4_2014_07_TRIAL.length !== 98) throw new Error('N4 2014-07 must contain 98 responses.');
if (new Set(N4_2014_07_TRIAL.map((question) => question.id)).size !== 98) throw new Error('N4 2014-07 contains duplicate question IDs.');
if (N4_2014_07_TRIAL.filter((question) => question.family !== 'listening').length !== 70) throw new Error('N4 2014-07 must contain 70 written responses.');
if (N4_2014_07_TRIAL.filter((question) => question.family === 'listening').length !== 28) throw new Error('N4 2014-07 must contain 28 listening responses.');
if (new Set(N4_2014_07_TRIAL.flatMap((question) => question.audio?.segmentId ?? [])).size !== 28) throw new Error('N4 2014-07 must contain 28 unique audio segments.');
