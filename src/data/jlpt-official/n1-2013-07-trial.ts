...
const DATASET = require('./n1-2013-07/exam.verified.json') as Dataset;
...
export const N1_2013_07_SESSION_KEY = 'jlpt:n1:2013-07:exam-03:session:v1';
export const N1_2013_07_TRIAL: readonly TrialQuestion[] = DATASET.questions.map((question) => ({
...
if (N1_2013_07_TRIAL.length !== 106) throw new Error('N1 2013-07 must contain 106 responses.');
if (new Set(N1_2013_07_TRIAL.map((question) => question.id)).size !== 106) throw new Error('N1 2013-07 contains duplicate question IDs.');
if (N1_2013_07_TRIAL.filter((question) => question.family !== 'listening').length !== 70) throw new Error('N1 2013-07 must contain 70 written responses.');
if (N1_2013_07_TRIAL.filter((question) => question.family === 'listening').length !== 36) throw new Error('N1 2013-07 must contain 36 listening responses.');
if (new Set(N1_2013_07_TRIAL.flatMap((question) => question.audio?.segmentId ?? [])).size !== 35) throw new Error('N1 2013-07 must contain 35 unique audio segments.');
