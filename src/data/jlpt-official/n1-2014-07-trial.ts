// Source-verified candidate; Simulator/audio review remains pending.
...
const DATASET = require('./n1-2014-07/exam.candidate.json') as Dataset;
...
export const N1_2014_07_SESSION_KEY = 'jlpt:n1:2014-07:exam-05:session:v1';
export const N1_2014_07_TRIAL: readonly TrialQuestion[] = DATASET.questions.map((question) => ({
...
if (N1_2014_07_TRIAL.length !== 107) throw new Error('N1 2014-07 must contain 107 responses.');
if (new Set(N1_2014_07_TRIAL.map((question) => question.id)).size !== 107) throw new Error('N1 2014-07 contains duplicate question IDs.');
if (N1_2014_07_TRIAL.filter((question) => question.family !== 'listening').length !== 70) throw new Error('N1 2014-07 must contain 70 written responses.');
if (N1_2014_07_TRIAL.filter((question) => question.family === 'listening').length !== 37) throw new Error('N1 2014-07 must contain 37 listening responses.');
if (new Set(N1_2014_07_TRIAL.flatMap((question) => question.audio?.segmentId ?? [])).size !== 36) throw new Error('N1 2014-07 must contain 36 unique audio segments.');
