...
const catalog = fs.readFileSync(path.join(root,'src/data/jlpt-official/jlpt-exam-catalog.ts'),'utf8');
const structuredIds = [...registry.matchAll(/id: '(n[1-4]-(\d{4})-(07|12)-exam-\d+)'/g)].map(m=>m[1]);
const pendingIds = [...catalog.matchAll(/'(n[1-4]-(\d{4})-(07|12))'/g)].map(m=>m[1]);
const rows = structuredIds.map((examId)=>{const [level,year,month]=examId.split('-');return {level:level.toUpperCase(),period:`${year}-${month}`,examId,questionText:true,options:true,correctAnswers:true,passages:true,transcripts:true,audio:true,audioBoundaries:true,renderer:'N1OfficialTrial',status:'structured_ready',sourceModule:`src/data/jlpt-official/${level}-${year}-${month}/`};});
for(const examId of pendingIds){const [level,year,month]=examId.split('-');rows.push({level:level.toUpperCase(),period:`${year}-${month}`,examId,questionText:false,options:false,correctAnswers:false,passages:false,transcripts:false,audio:false,audioBoundaries:false,renderer:'none (source package pending structured conversion)',status:'scanned_only',sourceModule:level==='n4'?`external-source:N4/${year}-${month}`:`src/data/jlpt-mock/${examId}-official.ts`});}
