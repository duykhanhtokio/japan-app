...
const examId = 'n1-2017-12-exam-12';
const reviewDir = 'docs/jlpt-workspace/conversion/n1-2017-12';
const root = 'assets/jlpt/n1/2017-12';
...
const sourceModule = fs.readFileSync('src/data/jlpt-mock/n1-2017-12-official.ts', 'utf8');
const key = (name) => sourceModule.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey = key('N1_2017_12_WRITTEN_KEY');
const listeningKey = key('N1_2017_12_LISTENING_KEY');
...
const writtenQuestions=written.map((q,index)=>{
  ...
  let passageId;
  if(q.passageJa){ passageId=passageIds.get(q.passageJa); if(!passageId){passageId=`n1-2017-12-passage-q${String(q.questionNumber).padStart(2,'0')}`;passageIds.set(q.passageJa,passageId);passages[passageId]={text:q.passageJa};} }
  return {questionId:`n1-2017-12-written-q${String(q.questionNumber).padStart(2,'0')}`,sectionId:'written',problemNumber:q.problemNumber,questionNumber:q.questionNumber,family:familyMap[q.family],sourceFamily:q.family,instructionJa:instructions[q.problemNumber]??'次の文章を読んで、問いに答えなさい。',promptJa:q.promptJa,passageId,options:optionObjects(q.options),correctOptionId:q.correctOptionId,verificationStatus:'candidate_source_transcription',source:{questionPage:q.sourcePage,questionPages:q.sourcePages,answerPage:1}};
});

const sha256=(path)=>createHash('sha256').update(fs.readFileSync(path)).digest('hex');
const audioPath=`${root}/audio/n1-2017-12.m4a`; const audioHash=sha256(audioPath);
assert.equal(audioHash,'2ac5cc6a65ce4461d6b67e168b5c95f498dfa4134f156f2ce6c7ccdc13e78325');
...
const out='src/data/jlpt-official/n1-2017-12/exam.candidate.json'; const serialized=`${JSON.stringify(data,null,2)}\n`;
...
console.log('N1 2017-12 built: 70 source-written + 35 listening responses; 34 candidate audio segments.');
