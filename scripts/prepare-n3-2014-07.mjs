...
import content from './n3-2014-07-written-content.mjs';

const source=fs.readFileSync('src/data/jlpt-mock/n3-2014-07-official.ts','utf8');
const key=(name)=>source.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey=key('N3_2014_07_WRITTEN_KEY'),listeningKey=key('N3_2014_07_LISTENING_KEY');
...
const page=n=>n<=17?2:n<=35?3:n<=48?4:n<=53?5:n<=61?6:n<=65?7:n<=70?8:n<=73?9:10;
...
const dir='docs/jlpt-workspace/conversion/n3-2014-07';fs.mkdirSync(dir,{recursive:true});
