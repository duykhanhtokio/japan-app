...
const reviewPath = 'docs/jlpt-workspace/conversion/n1-2015-12/written-page-02.review.json';
const questionPath = 'assets/jlpt/n1/2015-12/question/page-02.jpg';
...
const expectedHashes = new Map([
  [questionPath, 'c5318f40788403c0525c1de2670c8ba6f71efae6b2f77c7d82ccf9b6fe027085'],
...
const expectedKeys = [1, 2, 3, 4, 2, 1, 1, 3, 1, 2, 2, 4, 3, 4, 1, 2, 3, 4, 3];
...
const records = JSON.parse(await readFile(reviewPath, 'utf8'));
if (!Array.isArray(records) || records.length !== 19) fail(`expected 19 page-02 records, found ${records.length}`);
...
for (let index = 0; index < records.length; index += 1) {
  ...
  if (record.sourcePage !== 2) fail(`question ${questionNumber} has sourcePage ${record.sourcePage}`);
  if (![1, 2, 3].includes(record.problemNumber)) fail(`question ${questionNumber} has invalid problemNumber`);
  ...
  if ((record.problemNumber === 1 || record.problemNumber === 3) && !record.underlinedText) {
...
if (!expectedKeys.every((key, index) => declaredKeys[index] === key)) fail('page-02 keys differ from source declaration');

console.log('N1 2015-12 RECOVERY PASS: page 2 questions 1-19; four options each; unique IDs; 19/19 keys match 70-entry declaration; source hashes match');
