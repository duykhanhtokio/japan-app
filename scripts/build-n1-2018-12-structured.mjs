...
const examId = 'n1-2018-12-exam-14';
const reviewDir = 'docs/jlpt-workspace/conversion/n1-2018-12';
const root = 'assets/jlpt/n1/2018-12';
...
const sourceModule = fs.readFileSync('src/data/jlpt-mock/n1-2018-12-official.ts', 'utf8');
const key = (name) => sourceModule.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey = key('N1_2018_12_WRITTEN_KEY');
const listeningKey = key('N1_2018_12_LISTENING_KEY');
assert.equal(written.length, 70);
assert.equal(listeningReview.length, 34);
...
const writtenQuestions=written.map((q,index)=>{
  ...
  let passageId;
  if(q.passageJa){ passageId=passageIds.get(q.passageJa); if(!passageId){passageId=`n1-2018-12-passage-q${String(q.questionNumber).padStart(2,'0')}`;passageIds.set(q.passageJa,passageId);passages[passageId]={text:q.passageJa};} }
  return {questionId:`n1-2018-12-written-q${String(q.questionNumber).padStart(2,'0')}`,sectionId:'written',problemNumber:q.problemNumber,questionNumber:q.questionNumber,family:familyMap[q.family],sourceFamily:q.family,instructionJa:instructions[q.problemNumber]??'次の文章を読んで、問いに答えなさい。',promptJa:q.promptJa,passageId,options:optionObjects(q.options),correctOptionId:q.correctOptionId,verificationStatus:'candidate_source_transcription',source:{questionPage:q.sourcePage,questionPages:q.sourcePages,answerPage:1}};
});

const sha256=(path)=>createHash('sha256').update(fs.readFileSync(path)).digest('hex');
const audioPath=`${root}/audio/n1-2018-12.mp3`; const audioHash=sha256(audioPath);
assert.equal(audioHash,'0ce7be1fec7022a9e1075d5cf25a321e6c7c4ec4bead4740f1451d6a320a9550');
const listeningQuestions=listeningReview.map((q,index)=>{
  ...
  assert.ok(endMs>startMs&&endMs<=3174035);
  return {questionId:`n1-2018-12-p${q.problemNumber}-q${String(q.questionNumber).padStart(2,'0')}${q.responseSuffix?`-${q.responseSuffix}`:''}`,sectionId:'listening',problemNumber:q.problemNumber,questionNumber:q.questionNumber,responseSuffix:q.responseSuffix,family:'listening',instructionJa:listeningInstructions[q.problemNumber],promptJa:q.promptJa,options:optionObjects(q.options),correctOptionId:q.correctOptionId,source:q.source,answerVerificationStatus:'verified_against_source_key',transcriptVerificationStatus:'candidate_unverified',verificationStatus:'candidate_unverified',audio:{segmentId:`n1-2018-12-p${q.problemNumber}-q${String(q.questionNumber).padStart(2,'0')}`,startMs,endMs,transcriptJa:q.transcriptJa,transcriptSourcePages:q.source.answerScriptPages,timingVerificationStatus:'candidate_unverified',timingEvidence:`Local ffmpeg silencedetect candidate boundaries ${startSeconds.toFixed(3)} s and ${endSeconds.toFixed(3)} s were multiplied by 1000 and rounded to ${startMs} ms and ${endMs} ms. Timing is intentionally unverified pending later perceptual review.`,sourceAudioSha256:audioHash,candidateDate:'2026-09-21',humanReviewed:false,perceptualApproval:false,reviewDisposition:'needs_later_review'}};
});
const assets={}; for(const file of fs.readdirSync(`${root}/source-pages`).filter((x)=>/^page-\d+\.jpg$/.test(x)).sort()) assets[`${root}/source-pages/${file}`]=sha256(`${root}/source-pages/${file}`);
const questions=[...writtenQuestions,...listeningQuestions];
assert.equal(questions.length,104); assert.equal(new Set(questions.map((q)=>q.questionId)).size,104); assert.equal(new Set(listeningQuestions.map((q)=>q.audio.segmentId)).size,34);
const data={schemaVersion:1,examId,status:'candidate_complete',counts:{writtenResponses:70,listeningResponses:34,totalResponses:104,uniqueAudioSegments:34},blockers:[],source:{audioSha256:audioHash,assets},review:{audioTiming:'Candidate/unverified automated segmentation; humanReviewed and perceptualApproval remain false and later review is required.',transcripts:'Candidate integration text; source-script proofreading is deferred.',explanations:'Multilingual explanations and translations are deferred.'},passages,questions};
const out='src/data/jlpt-official/n1-2018-12/exam.candidate.json'; const serialized=`${JSON.stringify(data,null,2)}\n`;
...
console.log('N1 2018-12 built: 70 source-written + 34 listening responses; 34 candidate audio segments.');
