...
const examId = 'n1-2016-12-exam-10';
const reviewDir = 'docs/jlpt-workspace/conversion/n1-2016-12';
const root = 'assets/jlpt/n1/2016-12';
...
const sourceModule = fs.readFileSync('src/data/jlpt-mock/n1-2016-12-official.ts', 'utf8');
const key = (name) => sourceModule.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey = key('N1_2016_12_WRITTEN_KEY');
const listeningKey = key('N1_2016_12_LISTENING_KEY');
...
const writtenQuestions=written.map((q,index)=>{
  ...
  let passageId;
  if(q.passageJa){ passageId=passageIds.get(q.passageJa); if(!passageId){passageId=`n1-2016-12-passage-q${String(q.questionNumber).padStart(2,'0')}`;passageIds.set(q.passageJa,passageId);passages[passageId]={text:q.passageJa};} }
  return {questionId:`n1-2016-12-written-q${String(q.questionNumber).padStart(2,'0')}`,sectionId:'written',problemNumber:q.problemNumber,questionNumber:q.questionNumber,family:familyMap[q.family],sourceFamily:q.family,instructionJa:instructions[q.problemNumber]??'次の文章を読んで、問いに答えなさい。',promptJa:q.promptJa,passageId,options:optionObjects(q.options),correctOptionId:q.correctOptionId,verificationStatus:'verified_against_source_image',source:{questionPage:q.sourcePage,questionPages:q.sourcePages,answerPage:1}};
});

const sha256=(path)=>createHash('sha256').update(fs.readFileSync(path)).digest('hex');
const audioPath=`${root}/audio/n1-2016-12.m4a`; const audioHash=sha256(audioPath);
assert.equal(audioHash,'73a62db2cd4695babb39a60c402225e90186190eca70e3acbdc5f2fae68a0f00');
...
const out='src/data/jlpt-official/n1-2016-12/exam.candidate.json'; const serialized=`${JSON.stringify(data,null,2)}\n`;
...
console.log('N1 2016-12 built: 70 source-written + 37 listening responses; 36 candidate audio segments.');
