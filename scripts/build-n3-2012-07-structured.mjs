...
const examId='n3-2012-07-exam-01',reviewDir='docs/jlpt-workspace/conversion/n3-2012-07',root='assets/jlpt/n3/2012-07';
...
const sourceModule=fs.readFileSync('src/data/jlpt-mock/n3-2012-07-official.ts','utf8');
...
const writtenQuestions=written.map((q,index)=>{assert.equal(Number(q.correctOptionId),writtenKey[index]);assert.equal(q.options.length,4);return {questionId:`n3-2012-07-written-q${String(q.questionNumber).padStart(3,'0')}`,sectionId:'written',problemNumber:q.problemNumber,questionNumber:q.questionNumber,family:familyMap[q.family],sourceFamily:q.family,instructionJa:q.family.startsWith('reading')?'次の文章を読んで、問いに答えなさい。':'最もよいものを、1・2・3・4から一つ選びなさい。',promptJa:q.promptJa,options:optionObjects(q.options),correctOptionId:q.correctOptionId,verificationStatus:'candidate_source_transcription',source:{questionPage:q.sourcePage,questionPages:q.sourcePages,answerPage:1}};});
const sha256=(path)=>createHash('sha256').update(fs.readFileSync(path)).digest('hex');
const audioPath=`${root}/audio/n3-2012-07.mp3`,audioHash=sha256(audioPath);
assert.equal(audioHash,'cebbc344923dc575bd12556f6cbbee5e60d01a27263e28e5c9cc099087b5c600');
const listeningQuestions=listeningReview.map((q,index)=>{assert.equal(Number(q.correctOptionId),listeningKey[index]);const [s,e]=q.audioSeconds,startMs=Math.round(s*1000),endMs=Math.round(e*1000);assert.ok(endMs>startMs&&endMs<=2192797);return {questionId:`n3-2012-07-p${q.problemNumber}-q${String(q.questionNumber).padStart(2,'0')}${q.responseSuffix?`-${q.responseSuffix}`:''}`,sectionId:'listening',problemNumber:q.problemNumber,questionNumber:q.questionNumber,responseSuffix:q.responseSuffix,family:'listening',instructionJa:'音声を聞いて、最もよいものを一つ選びなさい。',promptJa:q.promptJa,options:optionObjects(q.options),correctOptionId:q.correctOptionId,source:q.source,answerVerificationStatus:'verified_against_source_key',transcriptVerificationStatus:'candidate_unverified',verificationStatus:'candidate_unverified',audio:{segmentId:`n3-2012-07-p${q.problemNumber}-q${String(q.questionNumber).padStart(2,'0')}`,startMs,endMs,transcriptJa:q.transcriptJa,transcriptSourcePages:q.source.answerScriptPages,timingVerificationStatus:'candidate_unverified',timingEvidence:`Candidate boundaries ${s.toFixed(3)} s and ${e.toFixed(3)} s were multiplied by 1000 and rounded to ${startMs} ms and ${endMs} ms.`,sourceAudioSha256:audioHash,candidateDate:'2026-09-21',humanReviewed:false,perceptualApproval:false,reviewDisposition:'needs_later_review'}};});
