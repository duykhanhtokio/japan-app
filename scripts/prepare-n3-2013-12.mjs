...
import content from './n3-2013-12-written-content.mjs';

const source=fs.readFileSync('src/data/jlpt-mock/n3-2013-12-official.ts','utf8');
const key=(name)=>source.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey=key('N3_2013_12_WRITTEN_KEY'),listeningKey=key('N3_2013_12_LISTENING_KEY');
...
const page=n=>n<=17?2:n<=35?3:n<=48?4:n<=53?5:n<=61?6:n<=65?7:n<=72?8:9;
...
const dir='docs/jlpt-workspace/conversion/n3-2013-12';fs.mkdirSync(dir,{recursive:true});
