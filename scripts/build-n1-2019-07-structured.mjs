...
const examId = 'n1-2019-07-exam-15';
const reviewDir = 'docs/jlpt-workspace/conversion/n1-2019-07';
const root = 'assets/jlpt/n1/2019-07';
...
const sourceModule = fs.readFileSync('src/data/jlpt-mock/n1-2019-07-official.ts', 'utf8');
const key = (name) => sourceModule.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey = key('N1_2019_07_WRITTEN_KEY');
const listeningKey = key('N1_2019_07_LISTENING_KEY');
...
const writtenQuestions=written.map((q,index)=>{
  ...
  let passageId;
  if(q.passageJa){ passageId=passageIds.get(q.passageJa); if(!passageId){passageId=`n1-2019-07-passage-q${String(q.questionNumber).padStart(2,'0')}`;passageIds.set(q.passageJa,passageId);passages[passageId]={text:q.passageJa};} }
  return {questionId:`n1-2019-07-written-q${String(q.questionNumber).padStart(2,'0')}`,sectionId:'written',problemNumber:q.problemNumber,questionNumber:q.questionNumber,family:familyMap[q.family],sourceFamily:q.family,instructionJa:instructions[q.problemNumber]??'次の文章を読んで、問いに答えなさい。',promptJa:q.promptJa,passageId,options:optionObjects(q.options),correctOptionId:q.correctOptionId,verificationStatus:'candidate_source_transcription',source:{questionPage:q.sourcePage,questionPages:q.sourcePages,answerPage:1}};
});
...
console.log('N1 2019-07 built: 69 source-written + 35 listening responses; 34 candidate audio segments.');
