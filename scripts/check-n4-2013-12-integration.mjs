...
const root='docs/jlpt-workspace/conversion/n4-2013-12';
...
const dataset=JSON.parse(fs.readFileSync('src/data/jlpt-official/n4-2013-12/exam.candidate.json'));
...
assert.equal(manifest.examId,'n4-2013-12-exam-04');
...
assert.ok(written.records.every(record=>record.answerKeyPage===14&&record.answerAudit==='checked_against_page_14_key'));
assert.ok(listening.records.every(record=>record.answerKeyPage===14&&record.answerAudit==='checked_against_page_14_key'));
assert.deepEqual(written.records.map(record=>Number(record.correctOptionId)),[3,2,2,4,1,1,3,4,2,1,4,3,2,1,4,1,3,2,1,2,3,4,2,4,1,2,1,4,3,2,4,4,2,1,1,3,1,4,1,4,4,2,4,2,2,3,2,1,3,1,2,1,3,3,4,4,1,1,3,2,2,1,3,4,2,1,2,4,3,4]);
assert.deepEqual(listening.records.map(record=>Number(record.correctOptionId)),[3,2,4,2,4,3,4,3,3,2,4,3,2,1,1,2,2,3,1,2,1,2,3,3,1,1,3,2]);
assert.deepEqual(listening.records.map(record=>[record.timingMs.start,record.timingMs.end]),[
  [201240,268640],[268640,335820],[335820,415440],[415440,504780],[504780,609300],[609300,717700],[717700,799620],[799620,871920],
  [1012360,1116480],[1116480,1218460],[1218460,1322300],[1322300,1427700],[1427700,1539960],[1539960,1671540],[1671540,1820400],
  [1904360,1941320],[1941320,1976920],[1976920,2014260],[2014260,2052040],[2052040,2096500],
  [2183100,2213820],[2213820,2249620],[2249620,2279520],[2279520,2312620],[2312620,2347240],[2347240,2380140],[2380140,2413680],[2413680,2456529],
]);
let previousEnd=0;
...
assert.equal(dataset.examId,'n4-2013-12-exam-04');
...
assert.match(registry,/id: 'n4-2013-12-exam-04'/);
assert.match(registry,/N4_2013_12_TRIAL/);
assert.doesNotMatch(catalog,/'n4-2013-12'/);
assert.equal(hash('assets/jlpt/n4/2013-12/audio/n4-2013-12.mp3'),manifest.runtimeAudio.sha256);
console.log('N4 2013-12 STRUCTURED INTEGRATION PASS: 70 written and 28 listening responses are registered; 28 timings remain candidate_unverified and require later perceptual review.');
