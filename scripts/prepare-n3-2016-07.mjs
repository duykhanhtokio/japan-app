...
import content from './n3-2016-07-written-content.mjs';
const source=fs.readFileSync('src/data/jlpt-mock/n3-2016-07-official.ts','utf8');
const key=name=>source.match(new RegExp(`${name} = \\[([\\s\\S]*?)\\]`))[1].match(/\d+/g).map(Number);
const writtenKey=key('N3_2016_07_WRITTEN_KEY'),listeningKey=key('N3_2016_07_LISTENING_KEY');
...
const page=n=>n<=13?2:n<=27?3:n<=34?4:n<=43?5:n<=48?6:n<=53?6:n<=58?7:n<=59?8:n<=61?9:n<=62?10:n<=65?11:n<=68?12:n<=71?13:14;
...
const duration=1999.307755,counts=[6,6,3,4,9],step=(duration-30)/28;let k=0;const listening=[];
for(let p=1;p<=5;p++)for(let i=1;i<=counts[p-1];i++){const start=15+k*step,end=15+(k+1)*step;listening.push({problemNumber:p,questionNumber:i,promptJa:`問題${p}・質問${i}`,options:Array.from({length:p>=4?3:4},(_,j)=>String(j+1)),correctOptionId:String(listeningKey[k++]),audioSeconds:[+start.toFixed(3),+end.toFixed(3)],transcriptJa:`問題${p}の質問${i}です。音声原本から統合した候補区間です。詳細な書き起こしの校閲は後続レビューで行います。`,source:{questionPages:[Math.min(19,14+p)],answerScriptPages:[Math.min(14,9+p)]},candidateStatus:'candidate_unverified',humanReviewed:false,perceptualApproval:false,reviewDisposition:'needs_later_review'});}
const dir='docs/jlpt-workspace/conversion/n3-2016-07';fs.mkdirSync(dir,{recursive:true});
...
console.log(`prepared ${written.length} written and ${listening.length} listening responses; candidate window ${duration}s`);
