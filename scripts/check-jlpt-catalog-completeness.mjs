...
const structured = [...registry.matchAll(/id: '(n[1-4]-\d{4}-(?:07|12)-exam-\d+)'/g)].map((match) => match[1]);
const pending = [...metadata.matchAll(/'n([1-4])-(\d{4})-(07|12)'/g)].map((match) => `n${match[1]}-${match[2]}-${match[3]}`);
...
if (allIds.length !== 59) failures.push(`catalogTotal expected 59, found ${allIds.length}`);
if (new Set(allIds).size !== allIds.length) failures.push('duplicate exam IDs found');
if (counts.N1 !== 15 || counts.N2 !== 13 || counts.N3 !== 17 || counts.N4 !== 9) failures.push(`official counts invalid: ${JSON.stringify(counts)}`);
...
console.log('JLPT CATALOG COMPLETENESS PASS: 59 entries = N1 official 15 + N2 official 13 + N3 official 17 + N4 official 9 + mocks 5.');
