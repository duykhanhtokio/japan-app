...
if (total !== 65 || new Set(officialPeriods).size !== 60 || JSON.stringify(actualN4) !== JSON.stringify(expectedN4) || JSON.stringify(actualN5) !== JSON.stringify(expectedN5) || invalidRuntime.length) {
...
console.log(`JLPT INVENTORY PASS: catalog 65; structured official ${structuredOfficial}; mock ready ${mockReady}; pending visible ${scannedOnly}; exact N4 periods 8; exact N5 candidate periods 7; scanned/Royal exam runtime 0`);
