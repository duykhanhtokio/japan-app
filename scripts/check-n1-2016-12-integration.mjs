...
const rebuilt = spawnSync(process.execPath,['scripts/build-n1-2016-12-structured.mjs','--check'],{encoding:'utf8'});
assert.equal(rebuilt.status,0,rebuilt.stderr);
const data=JSON.parse(fs.readFileSync('src/data/jlpt-official/n1-2016-12/exam.candidate.json','utf8'));
const source=fs.readFileSync('src/data/jlpt-mock/n1-2016-12-official.ts','utf8');
...
assert.equal(data.examId,'n1-2016-12-exam-10'); assert.deepEqual(data.counts,{writtenResponses:70,listeningResponses:37,totalResponses:107,uniqueAudioSegments:36});
assert.deepEqual(written.map(q=>Number(q.correctOptionId)),key('N1_2016_12_WRITTEN_KEY')); assert.deepEqual(listening.map(q=>Number(q.correctOptionId)),key('N1_2016_12_LISTENING_KEY'));
...
console.log('N1 2016-12 INTEGRATION PASS: 70 source-written; 37 listening responses; 36 ordered candidate segments; keys, IDs, provenance, and non-placeholder content validated.');
